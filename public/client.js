/* =========================================================
   すごろくゲーム  client.js
   バージョン: v3.6.5
   日付: 2026-06-21（日）04:57 JST
   v3.6.5での変更点:
     - 追分経由の重なりを解消。沼ノ端を最下段へ大きく下げ、
       植苗→南千歳→…→北広島を x 固定の縦一直線で真上へ。
       北広島→上野幌→新札幌→平和は左上へ流して白石へ接続。
       これで新札幌付近の重なりを解消。
     - その他（座標方式・描画・ロジック）は v3.6.4 から変更なし。
   --- 以下 過去履歴 ---
   ...
   ※ server.js v3.5 / index.html v3.6.5 とセットで使うこと
   ========================================================= */
...
// 白石(SHI)より下の帯のみ使用。栗山(右・中段)→右下へ追分→左へ→
// 沼ノ端を最下段へ→植苗〜北広島を縦一直線で真上→上野幌・新札幌・平和で白石へ。
// 末尾(平和)の次に白石へ接続。
const RAW_OIWAKE_BRANCH = [
  { x: SHI_X + 1850, y: SHI_Y - 250 },  // 0 栗山（岩見沢経由と同地点）
  { x: SHI_X + 1980, y: SHI_Y - 80 },   // 1 由仁
  { x: SHI_X + 2060, y: SHI_Y + 100 },  // 2 古山
  { x: SHI_X + 2090, y: SHI_Y + 290 },  // 3 三川
  { x: SHI_X + 2090, y: SHI_Y + 480 },  // 4 追分
  { x: SHI_X + 1870, y: SHI_Y + 620 },  // 5 安平
  { x: SHI_X + 1630, y: SHI_Y + 680 },  // 6 早来
  { x: SHI_X + 1400, y: SHI_Y + 740 },  // 7 遠浅
  { x: SHI_X + 1180, y: SHI_Y + 800 },  // 8 沼ノ端（最下段へ大きく下げる）
  { x: SHI_X + 1180, y: SHI_Y + 600 },  // 9 植苗（沼ノ端の真上）
  { x: SHI_X + 1180, y: SHI_Y + 430 },  // 10 南千歳（真上）
  { x: SHI_X + 1180, y: SHI_Y + 290 },  // 11 千歳（真上）
  { x: SHI_X + 1180, y: SHI_Y + 150 },  // 12 長都（真上）
  { x: SHI_X + 1180, y: SHI_Y + 10 },   // 13 サッポロビール庭園（真上）
  { x: SHI_X + 1180, y: SHI_Y - 130 },  // 14 恵庭（真上）
  { x: SHI_X + 1180, y: SHI_Y - 270 },  // 15 恵み野（真上）
  { x: SHI_X + 1180, y: SHI_Y - 410 },  // 16 島松（真上）
  { x: SHI_X + 1180, y: SHI_Y - 550 },  // 17 北広島（真上・縦ライン頂点）
  { x: SHI_X + 880,  y: SHI_Y - 550 },  // 18 上野幌（北広島から左へ）
  { x: SHI_X + 580,  y: SHI_Y - 480 },  // 19 新札幌（さらに左・少し下）
  { x: SHI_X + 300,  y: SHI_Y - 400 },  // 20 平和（白石へ向け左下）
];
